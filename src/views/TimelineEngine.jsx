...
const TypewriterText = ({ text, speed = 16 }) => {
  ...
  return (
    <span>
      ...
      {!done && (
        <span
          className="inline-block w-[2px] h-[1em] ml-0.5 align-middle bg-cyan-400"
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
            {movies.map(m => (
              <button
                ...
                className={`px-4 py-2 rounded-xl border text-xs font-medium transition-all whitespace-nowrap ${activeMovieId === m.id ? 'bg-cyan-500/20 border-cyan-500 text-cyan-400' : 'bg-black/40 border-white/10 text-gray-400 hover:text-white'}`}
              >
                {m.title}
              </button>
            ))}
        ...
        <div className="relative z-10 w-full max-w-lg bg-black/40 backdrop-blur-xl border border-white/10 p-10 rounded-3xl shadow-2xl flex flex-col gap-6">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <p className="text-gray-300 leading-relaxed mb-6 text-sm min-h-[4rem]">
                <TypewriterText key={currentNodeId} text={currentNode.description} speed={16} />
              </p>
              ...
              <div className="flex flex-col gap-3 mt-2">
                {isTerminalNode ? (
                  <button
                    ...
                    onMouseEnter={e => e.currentTarget.style.boxShadow = '0 0 18px 2px rgba(6,182,212,0.28)'}
                  ...
                  choicesList.map((choice, idx) => {
                    ...
                    return (
                      <button
                        ...
                        style={{ transition: 'background 0.2s, border-color 0.2s, box-shadow 0.2s' }}
                        onMouseEnter={e => {
                          e.currentTarget.style.boxShadow = isResetAction
                            ? '0 0 18px 2px rgba(6,182,212,0.25)'
                            : '0 0 16px 2px rgba(0,242,254,0.18)';
                        }}
                        onMouseLeave={e => e.currentTarget.style.boxShadow = 'none'}
                      >
                        {choice.label}
              ...
              <div className="mt-4 pt-4 border-t border-white/10">
                <button
                  ...
                  style={{ transition: 'background 0.2s, border-color 0.2s, box-shadow 0.2s' }}
                  onMouseEnter={e => e.currentTarget.style.boxShadow = '0 0 14px 1px rgba(6,182,212,0.2)'}
                  onMouseLeave={e => e.currentTarget.style.boxShadow = 'none'}
                >
                  View Timeline Map
                </button>
